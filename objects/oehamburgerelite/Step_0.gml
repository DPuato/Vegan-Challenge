...
if die=false
{
	...
	if hurt=false
	{
		...
		if hsp=0
		{sprite_index=hamburger_elite_enemy_idle}
		else
		{sprite_index=hamburger_elite_enemy_setal}
		if air {image_speed=0} else {image_speed=1}
	}
	else
	{
		hsp=0;
		if sprite_index!=hamburger_elite_enemy_rogy
		{
			sprite_index=hamburger_elite_enemy_rogy;
			image_index=0;
			image_speed=1;
		}
	}
...
{
	hsp=0;
	if sprite_index!=hamburger_elite_enemy_dead
	{
		audio_play_sound(snd_hamburger_halal,0,0);
		sprite_index=hamburger_elite_enemy_dead;
		image_index=0
	}
	image_speed=1
}
